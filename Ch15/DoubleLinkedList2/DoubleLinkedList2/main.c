...
//  DoubleLinkedList2
//
//  Created by DongGyu Park on 2016. 7. 6..
...
void insert_first(int data){
    ...
    // 연결리스트가 비어있으면
    if(isEmpty())
        head = last = new_node;  // head와 last가 새노드를 참조
    else {
...
void insert_last(int data){
    ...
    if(isEmpty())
        head = last = new_node;    // 새 노드를 마지막 노드로 지정함
    else {
        // 마지막 노드의 next가 새 노드를 참조하고
        last->next = new_node;
        // 새 노드의 prev가 last 노드를 참조함
        new_node->prev = last;
        // 새 노드가 last 노드가 참조함
        last = new_node;
    }
...
void delete(int key)
{
    ...
    if(current == last){    // last 포인터를 변경한다
        last = current->prev;
    }else {
